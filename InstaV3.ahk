...
Gui, Add, Picture, x0 y0 w720 h-750 , C:\Users\A\Desktop\InstaAdvanced\instaicon.jpg
Gui, Add, Picture, x0 y10 w450 h400 , C:\Users\A\Desktop\InstaAdvanced\instaicon.jpg
Gui, Add, Picture, x222 y230 w190 h70 , C:\Users\A\Desktop\InstaAdvanced\InstaMenager.png
Gui, 1: Add, CheckBox, x10 y10 Checked vCheck1 gAlways_On_Top, Zawsze na wierzchu
Gui, Add, Edit, x212 y30 w220 h130 vEB_Bulk_Save gBulk_Text_Edit, Wczytaj...
Gui, Add, DropDownList, x42 y-50 w140 h90 , DropDownList
Gui, Add, Text, x12 y10 w130 h20 +Left, Podaj ile like na hashtag:
Gui, Add, DropDownList, x22 y30 w100 h30 , DropDownList
Gui, Add, Text, x212 y10 w220 h20 , Podaj hashtagi:
Gui, Add, Text, x12 y70 w130 h20 , Followback po cyklu:
Gui, Add, CheckBox, x32 y90 w80 h30 , Wszystkich
Gui, Add, Link,x32 y150, Click <a href="www.instagram.com">here</a> to launch Instagram.
Gui, Add, Button, x212 y170 w220 h30 gzaladujH, Za³aduj #
;Gui, Add, MonthCal, vMyCalendar x-8 y230 w190 h160 ,
Gui, Add, MonthCal, vMyCal x-8 y230 w190 h160 , 20180101-20200108
Gui, Add, Text, x2 y200 w190 h30 , Ustaw plan tygodniowy:
Gui, Add, Tab2, x192 y200 w250 h110 , Tab1|Tab2
Gui, Add, Text, x262 y310 w120 h20 , Zacznij rozdawac like!
Gui, Add, Button,g x262 y330 w120 h40 gDo_Something1, Start!
;Gui, Add, Picture, x662 y440 w60 h90 , C:\Users\A\Desktop\InstaAdvanced\instaicon.jpg
;Gui, Add, Picture, x12 y10 w540 h490 , C:\Users\A\Desktop\InstaAdvanced\instaicon.jpg
Gui, Show,x0 y604 w443 h396, InstaBot 1.3
Clipboard = %Check1%
hastag:=Check1
x::ExitApp
z::Pause
...
zaladujH:
    FileRead, Temp, hajpowe.txt
    GuiControl,,EB_Bulk_Save, %Temp%
    return
...
;ssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss
strona_startowa(){
  Text:="|<>*187$16.0M03k0TU3b0QC3UQQ0vk3z0DA0kk330AA0kk33zwDzm"

if (ok:=FindText(118, 52, 150000, 150000, 0, 0, Text))
{
  CoordMode, Mouse
...
insta_button(){
Text:="|<>*156$22.3zz0zzz7U0SM00PU0Bw0knkDkD1nUwA33kkAD60MwM1XkkAD30kw7C3kDkD0A0y007M00Ns07Xzzw3zz2"

if (ok:=FindText(479, 110, 150000, 150000, 0, 0, Text))
{
  CoordMode, Mouse
  X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
  sleep,200
...
wykrywacz1(){
  if (ok:=FindText(879, 247, 150000, 150000, 0, 0, Text))
{
  CoordMode, Mouse
  X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
  return 1
...
powrot_menu(){
  Text:="|<>*137$71.w0201002000140412004400W808040088004HXrYDDDSvluT9cUcF1GWUIIEFFDEWSt57ccUWWWV54O+FFF1BB52++IIWWW1lvu4Lrchwx8000400000080000M000007UE"

if (ok:=FindText(986, 350, 150000, 150000, 0, 0, Text))
{
  CoordMode, Mouse
  X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
   Click, %X%, %Y%
...
x_podglad(){
  Text:="|<>*151$17.rzr7z47w47kQ71w47w0Tw1zw7zk7z07w47kQ71w47w4TwRzzk"

if (ok:=FindText(1872, 94, 150000, 150000, 0, 0, Text))
{
  CoordMode, Mouse
  X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
  sleep,200
...
return
 ...
 szukaj2(){
  Text:="|<>*190$53.003zzzzzy007zzzzzw00Dzzzzzs00Tzzzzzlk0kzzxzygk1DzzvzzEk2zzzrzz0U5y2Rgkq10BzwvHwi20SzNqDxKA0yxngy2bs1xrbMtp082vTgqr+004631ikI00Dzzzzzc00TzzzzyE00zzzzzlU01zzzzzzU"

if (ok:=FindText(946, 110, 150000, 150000, 0, 0, Text))
{
  CoordMode, Mouse
  X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
   Click, %X%, %Y%
...
wyszukiwarkaH()
    {
      Text:="|<>*227$52.00200000000800000000U0000000200000000800000000U00000D02000001z0/k00U0QA0s00200kM30008031UA7v4aSQA0c0AGk9Nk280lC0pzU8EX4kTFb0V4AHXB0A3AUn/8o00DbtwazE00U0000100200000A00800003U00U00000U"

if (ok:=FindText(1362, 122, 150000, 150000, 0, 0, Text))
{
  CoordMode, Mouse
  X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
   Click, %X%, %Y%
...
Text1:="|<>*220$10.D1zAAkP1gANlzVb0C"
szukaj2()
if (ok:=FindText(1362, 125, 150000, 150000, 0, 0, Text1))
{
  CoordMode, Mouse
  X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2

     ;Klikniecie w pole wpisania hasztagu
     sleep,500
     prawo:=X+10
   Click, %prawo%, %Y%
   Click, %prawo%, %Y%
  ...
  SLEEP,500
   ...
   ;Klikniecie w pierwsze zdjecie galeri
   skokX:=X-300
   skokY:=skok+450
	Click, %skokX%, %skokY%
	Click, %skokX%, %skokY%
...
serce_kropka()
	{
		Text:="|<heartdot>*182$22.1U60Tny31sAE30/000s001U006000M001k00B000a006A00kM060k0k1U601VU03A007U00A00000000000000000000000000000000A001s007U00A08"
		if (ok:=FindText(1789, 170, 150000, 150000, 0, 0, Text))
			{
			CoordMode, Mouse
			X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
			MouseMove, %X%, %Y%
...
like_back()
	{
Text:="|<>*185$21.U10Q080U104V/XY9WQV8PY93QV8PaNWQTDXU"

if (ok:=FindText(1523, 273, 150000, 150000, 0, 0, Text))
{
  CoordMode, Mouse
  X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
  SKOK:=x-25
...
like()
	{
Text1:="|<heart>*130$24.1s7U46M883k4E1U2E002U001U001U001U001U001E002E0028004400A600M300k0U100E200840048002E001U0U"

if (ok:=FindText(1439, 593, 150000, 150000, 0, 0, Text1))
{
  CoordMode, Mouse
  X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
;  Sleep, 200
...
next()
	{
		Text:="|<next>*130$13.7zVzkTw7z1zkTw7z1zkTw7z1zUzUTUTUTUTUTUTUTUTkTsTwTz"

if (ok:=FindText(512, 998, 150000, 150000, 0, 0, Text))
{
  CoordMode, Mouse
  X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
  Click, %X%, %Y%
